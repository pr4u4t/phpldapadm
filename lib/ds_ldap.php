...
*/
	...
	private function match_callback($match) { return chr(hexdec($match[0])); }
	...
	private function unescapeDN($dn) {
		...
		if (is_array($dn)) {
			...
			foreach ($dn as $key => $rdn)
				$a[$key] = preg_replace_callback('|\\\\([0-9A-Fa-f]{2})|',array($this,'match_callback'),$rdn);
...
#$a[$key] = preg_replace('/\\\([0-9A-Fa-f]{2})/e',"''.chr(hexdec('\\1')).''",$rdn);
		...
		{
			return preg_replace_callback('|\\\\([0-9A-Fa-f]{2})|',array($this,'match_callback'),$dn);
		}
	}
