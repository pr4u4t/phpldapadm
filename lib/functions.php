...
function masort(&$data,$sortby,$rev=0) {
	...
	if (empty($CACHE[$sortby])) {
		...
		eval('function '."MySortBy$sortby".' ($a,$b) { '.$code.' }');
		$CACHE[$sortby] = "MySortBy$sortby";
	}
 ...
 */
function my_password_hash($password_clear,$enc_type) {
	if (DEBUG_ENABLED && (($fargs=func_get_args())||$fargs='NOARGS'))
		debug_log('Entered (%%)',1,0,__FILE__,__LINE__,__METHOD__,$fargs);
...
function password_check($cryptedpassword,$plainpassword,$attribute='userpassword') {
	...
	switch($cypher) {
		...
		case 'sha':
			if (strcasecmp(my_password_hash($plainpassword,'sha'),'{SHA}'.$cryptedpassword) == 0)
				return true;
			else
				return false;
		...
		case 'md5':
			if( strcasecmp(my_password_hash($plainpassword,'md5'),'{MD5}'.$cryptedpassword) == 0)
				return true;
			else
				return false;
		...
		case 'sha512':
			if (strcasecmp(my_password_hash($plainpassword,'sha512'),'{SHA512}'.$cryptedpassword) == 0)
				return true;
			else
				return false;
...
function match_callback($match) { return chr(hexdec(trim($match[0],"\\"))); }
...
function dn_unescape($dn) {
	...
	if (is_array($dn)) {
		...
		foreach ($dn as $key => $rdn)
			$a[$key] = preg_replace_callback('|\\\\([0-9A-Fa-f]{2})|','match_callback',$rdn);

		return $a;

	} else {
		return preg_replace_callback('|\\\\([0-9A-Fa-f]{2})|','match_callback',$dn);
	}
}
